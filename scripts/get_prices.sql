...
select LTRIM(RTRIM(p.Article2)) AS ProdName,p.Article1 AS Article1, pl.PLName AS PriceName, p.UM AS UM,  mp.PriceMC AS ProdPrice
FROM  r_Crs cr
...
GROUP BY p.Article2,p.Article1,pl.PLName, p.UM, mp.PriceMC ;